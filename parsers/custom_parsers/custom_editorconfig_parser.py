...
class EditorconfigParser(BaseParser):
    ...
    def __init__(self, language_id: str='editorconfig', file_type: Optional
        [FileType]=None):
        super().__init__(language_id, file_type or FileType.CONFIG,
            parser_type=ParserType.CUSTOM)
        self.section_pattern = re.compile('^\\s*\\[(.*)\\]\\s*$')
    ...
    def _create_node(self, node_type: str, start_point: List[int],
        end_point: List[int], **kwargs) ->EditorconfigNode:
    ...
    def _parse_source(self, source_code: str) ->Dict[str, Any]:
        ...
        with ErrorBoundary(error_types=(ParsingError,), context=
            ...
            try:
                ...
                ast = self._create_node('editorconfig_file', [0, 0], [len(
                    lines) - 1, len(lines[-1]) if lines else 0], children=[])
                ...
                for i, line in enumerate(lines):
                    ...
                    if not line.strip():
                        if current_comments:
                            ast.children.append(self._create_node('comment',
                                [i - len(current_comments), 0], [i - 1, len
                                (current_comments[-1])], content='\n'.join(
                                current_comments)))
                            current_comments = []
                        continue
                    ...
                    if (section_match := self.section_pattern.match(line)):
                        ...
                        current_section = self._create_node('section',
                            line_start, line_end, pattern=pattern,
                            properties=[], children=[])
                        if current_comments:
                            current_section.metadata['comments'
                                ] = current_comments
                            current_comments = []
                    ...
                    if (property_match := self.property_pattern.match(line)):
                        ...
                        property_node = self._create_node('property',
                            line_start, line_end, key=key, value=value)
                        if current_comments:
                            property_node.metadata['comments'
                                ] = current_comments
                            current_comments = []
                        if current_section:
                ...
                if current_comments:
                    ast.children.append(self._create_node('comment', [len(
                        lines) - len(current_comments), 0], [len(lines) - 1,
                        len(current_comments[-1])], content='\n'.join(
                        current_comments)))
                return ast.__dict__
            except (ValueError, KeyError, TypeError) as e:
                log(f'Error parsing EditorConfig file: {e}', level='error')
                return self._create_node('editorconfig_file', [0, 0], [0, 0
                    ], error=str(e), children=[]).__dict__
    ...
    def extract_patterns(self, source_code: str) ->List[Dict[str, Any]]:
        ...
        with ErrorBoundary(error_types=(ProcessingError,), context=
            'EditorConfig pattern extraction'):
            try:
                patterns = []
                ...
                for section in section_patterns:
                    patterns.append({'name':
                        f"editorconfig_section_{section['pattern_type']}",
                        'content': section['pattern'], 'pattern_type':
                        PatternType.CONFIG_PATTERN, 'language': self.
                        language_id, 'confidence': 0.8, 'metadata': {'type':
                        'file_pattern', 'pattern': section['pattern'],
                        'pattern_type': section['pattern_type']}})
                property_patterns = self._extract_property_patterns(ast)
                for prop in property_patterns:
                    patterns.append({'name':
                        f"editorconfig_property_{prop['key']}", 'content':
                        f"{prop['key']} = {prop['value']}", 'pattern_type':
                        PatternType.CONFIG_PROPERTY, 'language': self.
                        language_id, 'confidence': 0.9, 'metadata': {'type':
                        'property', 'key': prop['key'], 'value': prop[
                        'value'], 'frequency': prop['frequency']}})
                style_patterns = self._extract_style_conventions(ast)
                for style in style_patterns:
                    patterns.append({'name':
                        f"editorconfig_style_{style['type']}", 'content':
                        style['description'], 'pattern_type': PatternType.
                        STYLE_CONVENTION, 'language': self.language_id,
                        'confidence': 0.85, 'metadata': {'type':
                        'style_convention', 'convention_type': style['type'
                        ], 'settings': style['settings']}})
                return patterns
            except (ValueError, KeyError, TypeError) as e:
                log(f'Error extracting EditorConfig patterns: {e}', level=
                    'error')
                return []
        ...
        def process_node(node):
            if isinstance(node, dict) and node.get('type') == 'section':
                ...
                for prop in node.get('properties', []):
                    properties.append({'key': prop.get('key', ''), 'value':
                        prop.get('value', '')})
                sections.append({'pattern': node.get('pattern', ''),
                    'pattern_type': PatternType.CODE_STRUCTURE, 'glob':
                    node.get('glob', ''), 'properties': properties})
        ...
        def process_node(node, current_section=None):
            if isinstance(node, dict):
                if node.get('type') == 'property':
                    prop = {'key': node.get('key', ''), 'value': node.get(
                        'value', ''), 'frequency': Counter(node.get('key',
                        ''))['']}
                    if current_section:
    ...
    def _extract_style_conventions(self, ast: Dict[str, Any]) ->List[Dict[
        str, Any]]:
        ...
        if indent_style or indent_size:
            style_patterns.append({'type': 'indentation', 'description':
                f"indent_style={indent_style or 'not_set'}, indent_size={indent_size or 'not_set'}"
                , 'settings': {'indent_style': indent_style, 'indent_size':
                indent_size}})
