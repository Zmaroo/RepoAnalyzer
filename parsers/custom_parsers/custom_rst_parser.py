...
class RstParser(BaseParser):
    ...
    def __init__(self, language_id: str='rst', file_type: Optional[FileType
        ]=None):
...
@handle_errors(error_types=(Exception,))
    ...
    def _create_node(self, node_type: str, start_point: List[int],
        end_point: List[int], **kwargs) ->RstNode:
    ...
    def _parse_source(self, source_code: str) ->Dict[str, Any]:
        ...
        """
        with ErrorBoundary(operation_name='parsing RST content',
            ...
            try:
                ...
                for i, line in enumerate(lines):
                    line_start = [i, 0]
                    line_end = [i, len(line)]
                    if self.patterns.get('section') and self.patterns['section'
                        ].match(line) and current_content:
                        section_title = current_content[-1]
                        section_level = self._get_section_level(line[0])
                        node = self._create_node('section', [i - 1, 0],
                            line_end, title=section_title, level=section_level)
                        while section_stack and section_stack[-1].metadata.get(
                            'level', 0) >= section_level:
                            section_stack.pop()
                        if section_stack:
                ...
                raise ParsingError(f'Failed to parse RST content: {str(e)}'
                    ) from e
        if error_boundary.error:
            log(f'Error in RST parser: {error_boundary.error}', level='error')
            return RstNode(type='document', start_point=[0, 0], end_point=[
                0, 0], error=str(error_boundary.error), children=[]).__dict__
    ...
    def extract_patterns(self, source_code: str) ->List[Dict[str, Any]]:
        ...
        """
        patterns = []
        with ErrorBoundary(operation_name='extracting RST patterns',
            ...
            try:
                ...
                for section in section_patterns:
                    patterns.append({'name':
                        f"rst_section_{section['name']}", 'content':
                        section['content'], 'pattern_type': PatternType.
                        DOCUMENTATION_STRUCTURE, 'language': self.
                        language_id, 'confidence': 0.9, 'metadata': {'type':
                        'rst_section', 'section_level': section.get('level',
                        1), 'section_count': section.get('count', 1)}})
                directive_patterns = self._extract_directive_patterns(ast_dict)
                for directive in directive_patterns:
                    patterns.append({'name':
                        f"rst_directive_{directive['type']}", 'content':
                        directive['content'], 'pattern_type': PatternType.
                        DOCUMENTATION_STRUCTURE, 'language': self.
                        language_id, 'confidence': 0.85, 'metadata': {
                        'type': 'rst_directive', 'directive_type':
                        directive['type'], 'directives': directive.get(
                        'items', [])}})
                role_patterns = self._extract_role_patterns(ast_dict)
                for role in role_patterns:
                    patterns.append({'name': f"rst_role_{role['type']}",
                        'content': role['content'], 'pattern_type':
                        PatternType.DOCUMENTATION_REFERENCE, 'language':
                        self.language_id, 'confidence': 0.8, 'metadata': {
                        'type': 'rst_role', 'role_type': role['type'],
                        'roles': role.get('items', [])}})
                structure_patterns = self._extract_structure_patterns(ast_dict)
                for structure in structure_patterns:
                    patterns.append({'name':
                        f"rst_structure_{structure['type']}", 'content':
                        structure['content'], 'pattern_type': PatternType.
                        DOCUMENTATION_STRUCTURE, 'language': self.
                        language_id, 'confidence': 0.85, 'metadata': {
                        'type': 'document_structure', 'structure_type':
                        structure['type'], 'count': structure.get('count', 1)}}
                        )
            except (ValueError, KeyError, TypeError, AttributeError) as e:
                log(f'Error extracting RST patterns: {e}', level='error')
                raise ProcessingError(
                    f'Failed to extract RST patterns: {str(e)}') from e
        if error_boundary.error:
            log(f'Error in RST pattern extraction: {error_boundary.error}',
                level='error')
        return patterns
...
@handle_errors(error_types=(Exception,))
        ...
        patterns = []
        if section_levels:
            patterns.append({'name': 'section_hierarchy', 'content':
                f'Document has {sum(section_levels.values())} sections across {len(section_levels)} levels'
                , 'level': len(section_levels), 'count': sum(section_levels
                .values())})
            for level, count in section_levels.items():
                if count >= 2:
                    patterns.append({'name': f'level_{level}_sections',
                        'content':
                        f'Document has {count} level {level} sections',
                        'level': level, 'count': count, 'examples':
                        section_titles.get(level, [])[:3]})
        return patterns

    def _extract_directive_patterns(self, ast: Dict[str, Any]) ->List[Dict[
        str, Any]]:
...
@handle_errors(error_types=(Exception,))
            ...
            if isinstance(node, dict):
                if node.get('type') == 'directive':
                    ...
                    if directive_content:
                        directive_examples[directive_type].append(
                            directive_content)
                for child in node.get('children', []):
                    process_node(child)
        process_node(ast)
        ...
        for directive_type, count in directive_types.items():
            patterns.append({'type': directive_type, 'content':
                f"Document uses {count} '{directive_type}' directives",
                'items': directive_examples.get(directive_type, [])[:3]})
        return patterns
...
@handle_errors(error_types=(Exception,))
        ...
        for role_type, count in role_types.items():
            patterns.append({'type': role_type, 'content':
                f"Document uses {count} '{role_type}' roles", 'items':
                role_examples.get(role_type, [])[:3]})
        return patterns
...
@handle_errors(error_types=(Exception,))
        ...
        if structure_elements:
            patterns.append({'type': 'document_composition', 'content':
                f"Document structure: {', '.join(structure_elements)}",
                'count': len(structure_elements)})
        if type_counts.get('field', 0) >= 3:
            patterns.append({'type': 'field_list', 'content':
                f"Document uses field lists ({type_counts['field']} fields)",
                'count': type_counts['field']})
        if type_counts.get('admonition', 0) >= 2:
            patterns.append({'type': 'admonitions', 'content':
                f"Document uses admonitions ({type_counts['admonition']} instances)"
                , 'count': type_counts['admonition']})
        if type_counts.get('reference', 0) >= 3:
            patterns.append({'type': 'references', 'content':
                f"Document uses many references ({type_counts['reference']} instances)"
                , 'count': type_counts['reference']})
        if type_counts.get('include', 0) >= 2:
            patterns.append({'type': 'modular_documentation', 'content':
                f"Document uses modular structure with includes ({type_counts['include']} instances)"
