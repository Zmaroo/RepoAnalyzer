...
class XmlParser(BaseParser):
    ...
    def __init__(self, language_id: str='xml', file_type: Optional[FileType
        ]=None):
...
@handle_errors(error_types=(Exception,))
    ...
    def _create_node(self, node_type: str, start_point: List[int],
        end_point: List[int], **kwargs) ->XmlNode:
    ...
    def _process_element(self, element: ET.Element, path: List[str],
        ...
        if '}' in tag:
            ...
            namespace = None
        element_data = self._create_node('element', start_point, [
            start_point[0], start_point[1] + len(str(element))], tag=tag,
            attributes=dict(element.attrib), text=element.text.strip() if
            element.text else None, metadata={'namespace': namespace,
            'path': '.'.join(path + [tag])})
        for child in element:
            child_node = self._process_element(child, path + [tag], [
        ...
        with ErrorBoundary(error_types=(ParsingError,), context='XML parsing'):
            try:
                ...
                try:
                    ...
                    return XmlNode(type='document', start_point=[0, 0],
    ...
    def extract_patterns(self, source_code: str) ->List[Dict[str, Any]]:
        ...
        with ErrorBoundary(error_types=(ProcessingError,), context=
            'XML pattern extraction'):
            try:
                ast = self._parse_source(source_code)
                element_patterns = self._extract_element_patterns(ast)
                for element in element_patterns:
                    patterns.append({'name':
                        f"xml_element_{element['tag']}", 'content': element
                        ['content'], 'pattern_type': PatternType.STRUCTURE,
                        'language': self.language_id, 'confidence': 0.85,
                        'metadata': {'type': 'element', 'tag': element[
                        'tag'], 'child_count': element['child_count'],
                ...
                for attr in attribute_patterns:
                    patterns.append({'name':
                        f"xml_attribute_{attr['name']}", 'content': attr[
                        'content'], 'pattern_type': PatternType.ATTRIBUTE,
                        'language': self.language_id, 'confidence': 0.8,
                        'metadata': {'type': 'attribute', 'name': attr[
                        'name'], 'element': attr['element'], 'value_type':
                        attr['value_type']}})
                namespace_patterns = self._extract_namespace_patterns(ast)
                for ns in namespace_patterns:
                    patterns.append({'name':
                        f"xml_namespace_{ns['prefix'] or 'default'}",
                        'content': ns['content'], 'pattern_type':
                        PatternType.NAMESPACE, 'language': self.language_id,
                        'confidence': 0.9, 'metadata': {'type': 'namespace',
        ...
        def process_node(node):
            if isinstance(node, dict):
                ...
                for child in node.get('children', []):
                    process_node(child)
        process_node(ast)
        common_elements = [elem for elem in elements if element_counts[elem
            ['tag']] > 1]
        return common_elements or elements[:5]

    def _extract_attribute_patterns(self, ast: Dict[str, Any]) ->List[Dict[
        str, Any]]:
...
@handle_errors(error_types=(Exception,))
        ...
        def process_node(node):
            if isinstance(node, dict):
                if node.get('type') == 'element' and node.get('attributes'):
                    for attr_name, attr_value in node.get('attributes', {}
                        ).items():
                        value_type = 'string'
                        if isinstance(attr_value, (int, float)):
                            ...
                            value_type = 'boolean'
                        attributes.append({'name': attr_name, 'element':
                            node.get('tag', 'unknown'), 'content':
                            f'{attr_name}="{attr_value}"', 'value_type':
                            value_type})
                for child in node.get('children', []):
                    process_node(child)
        process_node(ast)
    ...
    def _extract_namespace_patterns(self, ast: Dict[str, Any]) ->List[Dict[
        str, Any]]:
@handle_errors(error_types=(Exception,))
            ...
            if isinstance(node, dict):
                if node.get('type') == 'element':
                    if (namespace := node.get('metadata', {}).get('namespace')
                        ...
                        namespaces.append({'prefix': None, 'uri': namespace,
                            'content': f'xmlns="{namespace}"'})
                    for attr_name, attr_value in node.get('attributes', {}
                        ).items():
                        if attr_name == 'xmlns':
                            namespaces.append({'prefix': None, 'uri':
                                attr_value, 'content': f'xmlns="{attr_value}"'}
                                )
                        elif attr_name.startswith('xmlns:'):
                            prefix = attr_name.split(':', 1)[1]
                            namespaces.append({'prefix': prefix, 'uri':
                                attr_value, 'content':
    ...
    def _extract_naming_patterns(self, source_code: str) ->List[Dict[str, Any]
        ...
        pascal_case_tags = 0
        kebab_case_tags = 0
        tag_pattern = re.compile('<([a-zA-Z][a-zA-Z0-9:-]*)')
        ...
        if max_count > 3:
            ...
            patterns.append({'name': 'xml_tag_naming_convention', 'content':
                f'Tag naming convention: {convention}', 'pattern_type':
                PatternType.NAMING_CONVENTION, 'language': self.language_id,
                'confidence': 0.5 + 0.3 * (max_count / (camel_case_tags +
                pascal_case_tags + kebab_case_tags)), 'metadata': {'type':
                'naming_convention', 'element_type': 'tag', 'convention':
                convention, 'camel_case_count': camel_case_tags,
                'pascal_case_count': pascal_case_tags, 'kebab_case_count':
                kebab_case_tags}})
        return patterns
