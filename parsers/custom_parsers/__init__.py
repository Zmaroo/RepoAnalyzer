        ...
        with ErrorBoundary(f'loading parser module {module_name}'):
            try:
                module = importlib.import_module(
                    f'parsers.custom_parsers.{module_name}')
                for name, obj in inspect.getmembers(module):
                    if inspect.isclass(obj) and issubclass(obj, BaseParser
                        ) and obj != BaseParser:
                        language_id = getattr(obj, 'LANGUAGE_ID', None
                            ) or module_name.replace('custom_', '')
                        _parsers[language_id] = obj
                        log(f'Registered custom parser for: {language_id}',
                            level='debug')
            except (ImportError, AttributeError, TypeError) as e:
                log(f'Failed to load custom parser {module_name}: {e}',
                    level='error')
...
def get_parser(language_id: str, file_type: Optional[FileType]=None
    ...
    if parser_class:
        actual_file_type = (file_type if file_type is not None else
            FileType.CODE)
...
CUSTOM_PARSER_CLASSES: Dict[str, Type[BaseParser]] = {'env': EnvParser,
    'plaintext': PlaintextParser, 'yaml': YamlParser, 'markdown':
    MarkdownParser, 'editorconfig': EditorconfigParser, 'graphql':
    GraphqlParser, 'nim': NimParser, 'ocaml': OcamlParser,
    'ocaml_interface': OcamlParser, 'cobalt': CobaltParser, 'xml':
    XmlParser, 'html': HtmlParser, 'ini': IniParser, 'json': JsonParser,
    'restructuredtext': RstParser, 'toml': TomlParser, 'asciidoc':
    AsciidocParser}
__all__ = ['CUSTOM_PARSER_CLASSES', 'EnvParser', 'PlaintextParser',
    'YamlParser', 'MarkdownParser', 'EditorconfigParser', 'GraphqlParser',
    'NimParser', 'OcamlParser', 'CobaltParser', 'XmlParser', 'HtmlParser',
    'IniParser', 'JsonParser', 'RstParser', 'TomlParser', 'AsciidocParser']
