...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__),
    '..')))
from utils.error_handling import handle_errors, ErrorBoundary, ProcessingError, AsyncErrorBoundary
...
class ExceptionIssue:
    ...
    def to_dict(self) ->Dict[str, Any]:
        ...
        return {'file_path': self.file_path, 'line_number': self.
            line_number, 'issue_type': self.issue_type, 'description': self
            .description, 'severity': self.severity, 'suggested_fix': self.
            suggested_fix}
...
class AnalysisResult:
    ...
    def add_issue(self, issue: ExceptionIssue) ->None:
        ...
        self.issue_count_by_type[issue.issue_type
            ] = self.issue_count_by_type.get(issue.issue_type, 0) + 1
        self.issue_count_by_severity[issue.severity
            ] = self.issue_count_by_severity.get(issue.severity, 0) + 1

    def to_dict(self) ->Dict[str, Any]:
        """Convert to dictionary for JSON serialization."""
        return {'issues': [issue.to_dict() for issue in self.issues],
            'files_analyzed': self.files_analyzed, 'issue_count_by_type':
            self.issue_count_by_type, 'issue_count_by_severity': self.
            issue_count_by_severity, 'total_issues': len(self.issues)}
...
class ExceptionPatternVisitor(ast.NodeVisitor):
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef) ->None:
        ...
        for decorator in node.decorator_list:
            ...
            if decorator_name in ('handle_errors', 'handle_async_errors',
                'with_retry'):
                has_error_handling = True
                self.decorated_functions.add(node.name)
                break
        if not has_error_handling:
            try_except_count = sum(1 for n in ast.walk(node) if isinstance(
                n, ast.Try))
            if try_except_count > 0:
                self.result.add_issue(ExceptionIssue(file_path=self.
                    file_path, line_number=node.lineno, issue_type=
                    'missing_decorator', description=
                    f"Function '{node.name}' contains try/except but no error handling decorator"
                    , severity='medium', suggested_fix=
                    f"Consider adding @handle_errors or @handle_async_errors decorator to function '{node.name}'"
                    ))
    ...
    def visit_AsyncFunctionDef(self, node: ast.AsyncFunctionDef) ->None:
        ...
        if not has_error_handling and self._contains_await_in_try(node):
            self.result.add_issue(ExceptionIssue(file_path=self.file_path,
                line_number=node.lineno, issue_type=
                'missing_async_decorator', description=
                f"Async function '{node.name}' has try/except with await but no handle_async_errors decorator"
                , severity='high', suggested_fix=
                f"Add @handle_async_errors decorator to async function '{node.name}'"
                ))
        self.generic_visit(node)
        self.function_stack.pop()
    ...
    def visit_Try(self, node: ast.Try) ->None:
        ...
        for handler in node.handlers:
            if handler.type is None:
                self.result.add_issue(ExceptionIssue(file_path=self.
                    file_path, line_number=handler.lineno, issue_type=
                    'bare_except', description=
                    'Bare except clause used without specifying exception types'
                    , severity='high', suggested_fix=
                    'Specify exception types to catch, avoid catching all exceptions'
                    ))
                ...
                if not self._is_in_error_boundary(node):
                    self.result.add_issue(ExceptionIssue(file_path=self.
                        file_path, line_number=handler.lineno, issue_type=
                        'broad_except', description=
                        'Catching Exception is too broad', severity=
                        'medium', suggested_fix=
                        'Use more specific exception types or ErrorBoundary context manager'
                        ))
        ...
        for handler in node.handlers:
            if len(handler.body) == 1 and isinstance(handler.body[0], ast.Pass
                ...
                self.result.add_issue(ExceptionIssue(file_path=self.
                    file_path, line_number=handler.lineno, issue_type=
                    'silent_except', description=
                    'Exception silently ignored with pass', severity='high',
                    suggested_fix=
                    'Add proper error handling or logging instead of pass'))
        self.generic_visit(node)

    def visit_With(self, node: ast.With) ->None:
        """Visit with statements to track ErrorBoundary usage."""
        for item in node.items:
            if isinstance(item.context_expr, ast.Call) and hasattr(item.
                context_expr, 'func'):
                if isinstance(item.context_expr.func, ast.Name):
                    if item.context_expr.func.id in ('ErrorBoundary',
                        'AsyncErrorBoundary'):
    ...
    def visit_Raise(self, node: ast.Raise) ->None:
        ...
        if node.exc and isinstance(node.exc, ast.Call) and isinstance(node.
            exc.func, ast.Name):
            exception_name = node.exc.func.id
            if exception_name == 'Exception':
                self.result.add_issue(ExceptionIssue(file_path=self.
                    file_path, line_number=node.lineno, issue_type=
                    'generic_raise', description=
                    'Raising generic Exception', severity='medium',
                    suggested_fix=
                    'Use a more specific exception type from error_handling module'
                    ))
        self.generic_visit(node)
...
def analyze_file(file_path: str, result: AnalysisResult) ->None:
    ...
    with ErrorBoundary(f'analyzing file {file_path}', error_types=(
        SyntaxError, ProcessingError)):
        try:
            ...
            for node in ast.walk(tree):
                ...
                if has_error_imports:
                    break
            try_except_count = sum(1 for n in ast.walk(tree) if isinstance(
                n, ast.Try))
            if try_except_count > 0 and not has_error_imports:
                result.add_issue(ExceptionIssue(file_path=file_path,
                    line_number=1, issue_type=
                    'missing_error_handling_import', description=
                    "File contains exception handling but doesn't import error_handling utilities"
                    , severity='medium', suggested_fix=
                    'Import error handling utilities: from utils.error_handling import handle_errors, ErrorBoundary'
                    ))
            if 'db/' in file_path and not '/test' in file_path:
                ...
                if not has_retry_imports and try_except_count > 0:
                    result.add_issue(ExceptionIssue(file_path=file_path,
                        line_number=1, issue_type='missing_retry_mechanism',
                        description=
                        "Database operations file doesn't use retry mechanism",
                        severity='medium', suggested_fix=
                        'Import and use retry utilities: from db.retry_utils import with_retry'
                        ))
        except SyntaxError as e:
            result.add_issue(ExceptionIssue(file_path=file_path,
                line_number=e.lineno or 1, issue_type='syntax_error',
                description=f'Syntax error: {str(e)}', severity='high',
                suggested_fix='Fix the syntax error to enable proper analysis')
                )
        except Exception as e:
            result.add_issue(ExceptionIssue(file_path=file_path,
                line_number=1, issue_type='analysis_error', description=
                f'Error analyzing file: {str(e)}', severity='low',
                suggested_fix=None))


@handle_errors(error_types=(ProcessingError,))
def analyze_codebase(directory: str, exclude_dirs: List[str]=None
    ) ->AnalysisResult:
...
def print_report(result: AnalysisResult, verbose: bool=False,
    severity_filter: Optional[str]=None, issue_type_filter: Optional[str]=
    None, file_filter: Optional[str]=None, group_by_file: bool=False,
    fix_mode: bool=False, interactive: bool=True) ->None:
    """Print a human-readable report of the analysis results."""
    filtered_issues = result.issues
    if severity_filter:
        filtered_issues = [i for i in filtered_issues if i.severity ==
            severity_filter.lower()]
    if issue_type_filter:
        filtered_issues = [i for i in filtered_issues if i.issue_type ==
            issue_type_filter]
    if file_filter:
        filtered_issues = [i for i in filtered_issues if file_filter in i.
            file_path]
    print('\n' + '=' * 80)
    ...
    print('\nIssues by type:')
    for issue_type, count in sorted(result.issue_count_by_type.items(), key
    ...
    if fix_mode and filtered_issues:
        ...
        print('=' * 80)
        sorted_issues = sorted(filtered_issues, key=lambda x: (x.file_path,
            x.line_number))
        if interactive:
    ...
    if group_by_file and filtered_issues:
        ...
        for file_path, issues in sorted(issues_by_file.items()):
            ...
            severity_counts = {}
            for issue in issues:
                severity_counts[issue.severity] = severity_counts.get(issue
                    .severity, 0) + 1
    ...
    if verbose:
        print('\nDetailed Issues:')
        for issue in sorted(filtered_issues, key=lambda x: (x.severity, x.
            file_path, x.line_number)):
            severity_marker = {'high': '!!!', 'medium': ' !!', 'low': '  !'
                }.get(issue.severity, '   ')
        ...
        high_severity = [i for i in filtered_issues if i.severity == 'high'
            ] if not severity_filter else filtered_issues
        if high_severity:
            print('\nHigh Severity Issues:') if not severity_filter else print(
                f"""
{severity_filter.capitalize()} Severity Issues:""")
            for issue in sorted(high_severity, key=lambda x: (x.file_path,
                x.line_number))[:10]:
                print(
            ...
            if len(high_severity) > 10:
                print(
                    f"  ... and {len(high_severity) - 10} more {severity_filter if severity_filter else 'high'} severity issues"
                    )
    print('\nRecommendations:')
    ...
    if result.issue_count_by_type.get('silent_except', 0) > 0:
        print(
            '  - Add proper error handling instead of silently ignoring exceptions'
            )
    if result.issue_count_by_type.get('missing_decorator', 0) > 0:
        print(
            '  - Add handle_errors or handle_async_errors decorators to functions with try/except'
            )
    if result.issue_count_by_type.get('missing_error_handling_import', 0) > 0:
        print(
            '  - Import error handling utilities in files with exception handling'
            )
...
def main():
    ...
    parser.add_argument('--issue-type', dest='issue_type', help=
        'Filter issues by type (e.g., missing_async_decorator, broad_except)')
    parser.add_argument('--file', help=
        'Filter issues to only those in files matching this pattern')
    parser.add_argument('--group-by-file', action='store_true', help=
        'Group issues by file')
    parser.add_argument('--fix-mode', action='store_true', help=
        'Mode showing issues with fix suggestions')
    parser.add_argument('--non-interactive', action='store_true', help=
        'Show all fixes at once without waiting for user input (use with --fix-mode)'
        )
    ...
    print_report(result, args.verbose, args.severity, args.issue_type, args
        .file, args.group_by_file, args.fix_mode, interactive)
    if args.output:
        filtered_issues = result.issues
        if args.severity:
            filtered_issues = [i for i in filtered_issues if i.severity ==
        ...
        if args.issue_type:
            filtered_issues = [i for i in filtered_issues if i.issue_type ==
                args.issue_type]
        if args.file:
            filtered_issues = [i for i in filtered_issues if args.file in i
                .file_path]
        if args.severity or args.issue_type or args.file:
            ...
            filtered_result.issue_count_by_severity = (result.
